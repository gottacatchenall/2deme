...
ggplot(subset(data, fitness == 0), aes(generation, global_ld_mean, group=(run_id), color=(as.factor(PATCH_K_MEAN)))) + geom_point() + geom_line()

ggplot(subset(data), aes(generation, fst_mean, group=(run_id), color=(as.factor(MEAN_LOCUS_WEIGHT)))) + geom_point() + geom_line()+ ylim(0,1)
